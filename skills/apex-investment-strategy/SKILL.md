...
Apex 投资策略 helps users build a local-first, optional-layer investment strategy workspace: research notes first, then a minimum SQLite price database, then backtests, paper wallet tracking, Action Packets, market-companion explanations, and a static web dashboard when requested.
...
- If discussing recent market moves, use current public sources and cite them. If no live web/search tool or user-provided source is available, say the explanation cannot be current.
